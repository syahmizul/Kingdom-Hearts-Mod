...
void InputSys::Initialize()
{
	...
	while (!Globals::g_SwapChain) // Wait for swap chain to get assigned by Present hook
		Sleep(1000);
	if (FAILED(Globals::g_SwapChain->GetDesc(&params)))
		throw std::runtime_error("[InputSys] Swap chain GetDesc failed.");

	m_hTargetWindow = params.OutputWindow;
...
{
	/*Utils::ConsolePrint(std::to_string(uMsg).c_str());*/
	switch (uMsg) {
